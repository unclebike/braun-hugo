...
const matchTimeWindow = (start, ruleStart, ruleEnd) => {
...
const isRuleApplicable = (rule, territoryId, date, startTime, bookingDateTime) => {
...
export async function calculateAdjustedPrice(db, serviceId, basePrice, territoryId, date, startTime) {
    const rulesResult = await db.prepare(`SELECT id, service_id, territory_id, rule_type, adjustment_type, adjustment_value, direction,
     ...
     ORDER BY created_at ASC`).bind(serviceId, territoryId).all();
