...
const slotFitsWindow = (slotStartMin, slotEndMin, startTime, endTime) => {
    const startMin = toMinutes(startTime);
    const endMin = toMinutes(endTime);
    return slotStartMin >= startMin && slotEndMin <= endMin;
};
export async function generateTimeslots(db, territoryId, date, durationMinutes, requiredProviderCount, requiredSkills) {
    const providerRows = await db.prepare(`SELECT tm.id
     FROM team_members tm
     JOIN team_member_territories tmt ON tmt.team_member_id = tm.id
     WHERE tmt.territory_id = ?
       AND tm.role = 'provider'
       AND tm.is_active = 1
       AND tm.can_be_auto_assigned = 1`).bind(territoryId).all();
    ...
    if (requiredSkills.length > 0 && providers.length > 0) {
        ...
        const skillRows = await db.prepare(`SELECT tms.team_member_id, COUNT(DISTINCT tms.skill_id) AS matched_skill_count
       FROM team_member_skills tms
       WHERE tms.team_member_id IN (${providers.map(() => '?').join(', ')})
         AND tms.skill_id IN (${placeholders})
       GROUP BY tms.team_member_id`).bind(...providers.map((p) => p.id), ...requiredSkills).all();
        const qualified = new Set((skillRows.results || [])
    ...
    const [weeklyRows, overrideRows] = await Promise.all([
        db.prepare(`SELECT team_member_id, day_of_week, start_time, end_time
       FROM provider_weekly_hours
       WHERE team_member_id IN (${weeklyPlaceholders})
         AND day_of_week = ?`).bind(...providerIds, dayOfWeek).all(),
        db.prepare(`SELECT team_member_id, is_available, start_time, end_time
       FROM provider_date_overrides
       WHERE team_member_id IN (${weeklyPlaceholders})
         AND date = ?`).bind(...providerIds, date).all(),
    ]);
