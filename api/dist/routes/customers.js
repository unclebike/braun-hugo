...
app.get('/', async (c) => {
    try {
        ...
        const addressRows = await db.prepare(`SELECT * FROM customer_addresses
       WHERE customer_id IN (${ids.map(() => '?').join(', ')})
       ORDER BY is_default DESC, created_at DESC`).bind(...ids).all();
...
app.post('/', async (c) => {
    try {
        ...
        const duplicate = await db.prepare(`SELECT id FROM customers
       ...
       LIMIT 1`).bind(email, email, phoneE164, phoneE164).first();
        ...
        await db.prepare(`INSERT INTO customers (id, first_name, last_name, email, phone, phone_e164, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`).bind(id, body.first_name, body.last_name, email, phone || null, phoneE164).run();
        for (const address of asArray(body.customer_addresses)) {
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`).bind(crypto.randomUUID(), id, address.line_1, address.line_2 || null, address.city, address.state, address.postal_code, address.lat ?? null, address.lng ?? null, address.is_default ? 1 : 0).run();
        }
...
app.patch('/:id', async (c) => {
    try {
       ...
       LIMIT 1`).bind(id, normalizedEmail !== undefined ? normalizedEmail : null, normalizedEmail !== undefined ? normalizedEmail : null, normalizedPhoneE164 !== undefined ? normalizedPhoneE164 : null, normalizedPhoneE164 !== undefined ? normalizedPhoneE164 : null).first();
        if (duplicate)
            return c.json({ error: 'Customer already exists' }, 409);
