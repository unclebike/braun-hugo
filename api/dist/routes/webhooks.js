...
app.post('/', async (c) => {
    try {
        ...
        await c.env.DB.prepare(`INSERT INTO webhooks (id, url, event_type, secret, is_active, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, datetime('now'), datetime('now'))`).bind(id, body.url, body.event_type, body.secret || crypto.randomUUID(), body.is_active === false ? 0 : 1).run();
