...
app.get('/', async (c) => {
    try {
        ...
        const [serviceRows, providerRows] = await Promise.all([
            db.prepare(`SELECT ts.territory_id, s.id as service_id, s.name
         FROM territory_services ts
         JOIN services s ON s.id = ts.service_id
         WHERE ts.territory_id IN (${placeholders})`).bind(...ids).all(),
            db.prepare(`SELECT tmt.territory_id, tm.id as team_member_id, tm.first_name, tm.last_name
         FROM team_member_territories tmt
         JOIN team_members tm ON tm.id = tmt.team_member_id
         WHERE tmt.territory_id IN (${placeholders})`).bind(...ids).all(),
        ]);
        const servicesByTerritory = new Map();
        ...
        const providersByTerritory = new Map();
...
app.get('/:id', async (c) => {
    try {
        ...
        const [services, providers] = await Promise.all([
         ...
         FROM territory_services ts
         JOIN services s ON s.id = ts.service_id
         WHERE ts.territory_id = ?`).bind(id).all(),
            db.prepare(`SELECT tm.id, tm.first_name, tm.last_name
         FROM team_member_territories tmt
         JOIN team_members tm ON tm.id = tmt.team_member_id
         WHERE tmt.territory_id = ?`).bind(id).all(),
        ]);
        return c.json({
            ...territory,
...
app.post('/', async (c) => {
    try {
        ...
        await db.prepare(`INSERT INTO territories
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`).bind(id, body.name, body.timezone || 'America/Toronto', body.service_area_type || 'zip', body.service_area_data || '{}', body.operating_hours || '{}', body.scheduling_policy || 'provider_based', body.max_concurrent_jobs ?? null, body.is_active === false ? 0 : 1).run();
