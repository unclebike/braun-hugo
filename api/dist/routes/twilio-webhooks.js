...
import { ensureSmsInboxMessage, getTwilioConfig, logSms, normalizePhoneE164, touchSmsInboxMessage, updateSmsStatus, validateTwilioSignature, } from '../services/twilio';
const app = new Hono();
const parseFormParams = (rawBody) => {
    const params = {};
...
app.post('/status', async (c) => {
    ...
    if (!config)
        return c.text('Not configured', 200);
...
app.post('/inbound', async (c) => {
    ...
    const customer = await db.prepare('SELECT id, first_name, last_name, email FROM customers WHERE phone_e164 = ? OR phone = ? ORDER BY updated_at DESC LIMIT 1').bind(phoneE164, from).first();
    const latestJob = customer?.id
        ? await db.prepare(`SELECT id FROM jobs
       WHERE customer_id = ?
    ...
    if (['STOP', 'STOPALL', 'UNSUBSCRIBE', 'CANCEL', 'END', 'QUIT'].includes(upperBody)) {
        if (customer) {
            await db.prepare("UPDATE customers SET sms_opted_out = 1, sms_opted_out_at = datetime('now'), updated_at = datetime('now') WHERE id = ?").bind(customer.id).run();
            console.log(`[twilio-webhook] Customer ${customer.id} opted out via ${upperBody}`);
        }
    }
    if (['START', 'YES', 'UNSTOP'].includes(upperBody)) {
        if (customer) {
            await db.prepare("UPDATE customers SET sms_opted_out = 0, sms_opted_out_at = NULL, updated_at = datetime('now') WHERE id = ?").bind(customer.id).run();
    ...
    c.executionCtx.waitUntil(enqueueAndDispatchPushEvent(db, {
        type: 'new_message',
