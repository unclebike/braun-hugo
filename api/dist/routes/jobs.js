...
const maybeAutoCreateInvoice = async (db, jobId) => {
    ...
    await db.prepare(`INSERT INTO invoices (id, invoice_number, job_id, customer_id, currency, amount_cents, subtotal_cents, tax_cents, discount_cents, total_cents, line_items_json, due_date, status, created_at, updated_at)
     VALUES (?, ?, ?, ?, 'CAD', ?, ?, 0, 0, ?, ?, ?, 'pending', datetime('now'), datetime('now'))`).bind(crypto.randomUUID(), invoiceNumber, jobId, job.customer_id, subtotal, subtotal, subtotal, JSON.stringify(effectiveLines), due.toISOString().split('T')[0]).run();
};
app.get('/', async (c) => {
    try {
        const db = c.env.DB;
        ...
        const jobs = await (params.length
            ? db.prepare(`SELECT j.*, c.first_name || ' ' || c.last_name as customer_name, s.name as service_name
           FROM jobs j
           JOIN customers c ON c.id = j.customer_id
           LEFT JOIN services s ON s.id = j.service_id
           ${clauses.length ? `WHERE ${clauses.join(' AND ')}` : ''}
           ORDER BY j.scheduled_date DESC, j.scheduled_start_time DESC`).bind(...params).all()
            : db.prepare(`SELECT j.*, c.first_name || ' ' || c.last_name as customer_name, s.name as service_name
           FROM jobs j
           JOIN customers c ON c.id = j.customer_id
           LEFT JOIN services s ON s.id = j.service_id
           ORDER BY j.scheduled_date DESC, j.scheduled_start_time DESC`).all());
        const ids = (jobs.results || []).map((j) => j.id);
        if (ids.length === 0)
            return c.json({ jobs: [] });
        const [providers, notes] = await Promise.all([
            db.prepare(`SELECT jp.job_id, tm.id, tm.first_name, tm.last_name
         FROM job_providers jp
         JOIN team_members tm ON tm.id = jp.team_member_id
         WHERE jp.job_id IN (${ids.map(() => '?').join(', ')})`).bind(...ids).all(),
            db.prepare(`SELECT id, job_id, content, created_at
         FROM job_notes
         WHERE job_id IN (${ids.map(() => '?').join(', ')})
         ORDER BY created_at DESC`).bind(...ids).all(),
        ]);
        const providersByJob = new Map();
...
app.get('/:id', async (c) => {
    try {
        ...
        const [providers, notes] = await Promise.all([
            db.prepare(`SELECT tm.id, tm.first_name, tm.last_name
         FROM job_providers jp
         JOIN team_members tm ON tm.id = jp.team_member_id
         WHERE jp.job_id = ?`).bind(id).all(),
...
app.post('/', async (c) => {
    try {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`).bind(id, body.customer_id, body.service_id || null, body.territory_id || null, body.customer_address_id || null, body.scheduled_date, body.scheduled_start_time, body.duration_minutes || 60, body.base_price_cents || 0, body.total_price_cents || body.base_price_cents || 0, JSON.stringify([buildServiceBaseLine(String(body.custom_service_name || 'Service'), Number(body.total_price_cents || body.base_price_cents || 0))]), body.custom_service_name || null, body.status || 'created').run();
