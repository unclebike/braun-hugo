...
import { buildServiceBaseLine, normalizeLine, parseEditableText, parsePriceLines, subtotalFromLines } from '../utils/line-items';
import { FormView, TableView } from '../views/components';
...
import { AddressSearchResults, CustomerSearchResults, JobWizardPage, JobWizardSwapBundle, parseWizardState } from '../views/job-wizard';
...
import { deactivatePushSubscription, enqueueTestPushNotificationAndPing, getPushSubscriptionStatus, getPushVapidPublicKey, pullPendingPushNotifications, upsertPushSubscription, } from '../services/notifications';
...
const getCompletedSmsTaskIds = async (db, jobId) => {
    if (!jobId)
        return [];
    ...
    try {
        ...
        for (const note of notes) {
            ...
            if (!noteRecord.completed)
                continue;
            const source = noteRecord.source;
            if (!source || typeof source !== 'object' || Array.isArray(source))
                continue;
            const sourceRecord = source;
            if (sourceRecord.type !== 'sms')
                continue;
...
const getAuthenticatedStaffEmail = (c) => {
    const auth = c.get('auth');
    if (!auth || auth.type !== 'cf_access' || !auth.email)
        return null;
    return auth.email;
};
...
const recomputeJobTotals = async (db, jobId) => {
...
const syncInvoiceFromJob = async (db, jobId) => {
    ...
    await db.prepare(`UPDATE invoices
     ...
     WHERE id = ?`).bind(JSON.stringify(lines), subtotal, total, total, invoice.id).run();
};
const writeInvoiceLines = async (db, invoiceId, lines) => {
    ...
    await db.prepare(`UPDATE invoices
     SET line_items_json = ?, subtotal_cents = ?, amount_cents = ?, total_cents = ?, updated_at = datetime('now')
     WHERE id = ?`).bind(JSON.stringify(lines), subtotal, total, total, invoiceId).run();
};
const parseImportedCustomers = (raw) => {
    const text = raw.trim();
    if (!text)
        return [];
...
app.get('/', async (c) => {
    ...
    const [todayJobs, weekJobs, totalCustomers, activeTerritories, activeProviders, pendingInvoices, upcomingJobs, recentBookings, recentMessages] = await Promise.all([
        db.prepare(`
...
app.post('/territories', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).bind(id, body.name, body.timezone || 'America/Toronto', body.service_area_type, body.service_area_data || '{}', body.scheduling_policy, body.is_active === 'on' ? 1 : 0).run();
...
app.post('/territories/:id', async (c) => {
    ...
    if (section === 'basic') {
      ...
      WHERE id = ?
    `).bind(body.name, body.timezone || 'America/New_York', body.scheduling_policy, body.is_active === 'on' ? 1 : 0, id).run();
        return c.redirect(`/admin/territories/${id}`);
    }
    await db.prepare(`
    UPDATE territories 
    ...
    WHERE id = ?
  `).bind(body.name, body.timezone || 'America/New_York', body.service_area_type, body.service_area_data || '{}', body.scheduling_policy, body.is_active === 'on' ? 1 : 0, id).run();
    return c.redirect(`/admin/territories/${id}`);
});
...
app.get('/territories/:id/area-panel/:type', async (c) => {
    ...
    if (panelType === 'zip')
        return c.html(ZipPanel({ tid: id, zipCodes }));
    if (panelType === 'radius')
        return c.html(RadiusPanel({ tid: id, areaData }));
    if (panelType === 'geofence')
        return c.html(GeofencePanel({ tid: id, areaData }));
...
app.get('/territories/:id', async (c) => {
    ...
    const id = c.req.param('id');
    if (id === 'new')
        return c.redirect('/admin/territories/new');
...
app.get('/categories', async (c) => {
    ...
    return c.html(TableView({
        ...
        rows,
        rawIds: (categories.results || []).map(cat => cat.id),
...
app.post('/categories', async (c) => {
    ...
    const body = await c.req.parseBody();
    const id = generateId();
    await db.prepare('INSERT INTO service_categories (id, name, sort_order) VALUES (?, ?, ?)').bind(id, body.name, parseInt(body.sort_order, 10) || 0).run();
    return c.redirect('/admin/categories');
});
app.get('/categories/:id', async (c) => {
    const db = c.env.DB;
    const id = c.req.param('id');
    const category = await db.prepare('SELECT * FROM service_categories WHERE id = ?').bind(id).first();
    if (!category)
        return c.redirect('/admin/categories');
    const fields = [
...
app.post('/categories/:id', async (c) => {
    ...
    const id = c.req.param('id');
    const body = await c.req.parseBody();
    await db.prepare("UPDATE service_categories SET name = ?, sort_order = ?, updated_at = datetime('now') WHERE id = ?").bind(body.name, parseInt(body.sort_order, 10) || 0, id).run();
    return c.redirect('/admin/categories');
});
...
app.get('/skills', async (c) => {
    ...
    return c.html(TableView({
        ...
        rows,
        rawIds: (skills.results || []).map(skill => skill.id),
...
app.post('/skills', async (c) => {
    ...
    const body = await c.req.parseBody();
    const id = generateId();
    await db.prepare('INSERT INTO skills (id, name, description) VALUES (?, ?, ?)').bind(id, body.name, body.description || null).run();
...
app.post('/skills/:id', async (c) => {
    ...
    const id = c.req.param('id');
    const body = await c.req.parseBody();
    await db.prepare("UPDATE skills SET name = ?, description = ?, updated_at = datetime('now') WHERE id = ?").bind(body.name, body.description || null, id).run();
    return c.redirect('/admin/skills');
});
...
app.get('/services/new', async (c) => {
    ...
    const fields = [
        { name: 'name', label: 'Name', required: true },
        { name: 'description', label: 'Description', type: 'textarea' },
        { name: 'category_id', label: 'Category', type: 'select', options: (categories.results || []).map(c => ({ value: c.id, label: c.name })) },
...
app.post('/services', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, body.name, body.description || null, body.category_id || null, Math.round(parseFloat(body.base_price || '0') * 100), parseInt(body.base_duration_minutes, 10) || 60, body.auto_assign_enabled === 'on' ? 1 : 0, body.auto_assign_method || 'balanced', body.is_active === 'on' ? 1 : 0).run();
...
app.post('/services/:id', async (c) => {
    ...
    if (section === 'basic') {
      ...
      WHERE id = ?
    `).bind(body.name, body.description || null, body.category_id || null, Math.round(parseFloat(body.base_price || '0') * 100), parseInt(body.base_duration_minutes, 10) || 60, body.auto_assign_enabled === 'on' ? 1 : 0, body.auto_assign_method || 'balanced', body.is_active === 'on' ? 1 : 0, id).run();
...
const renderServiceDetail = async (c, serviceId) => {
    const db = c.env.DB;
    ...
    const ruleList = (priceRules.results || []).map(r => ({
        ...
        days_of_week: r.days_of_week || undefined,
        ...
        territory_name: r.territory_name || undefined,
    }));
...
app.get('/services/:id', async (c) => {
    const id = c.req.param('id');
    if (id === 'new')
        return c.redirect('/admin/services/new');
...
app.post('/services/:id/modifiers', async (c) => {
    ...
    await db.prepare('INSERT INTO service_modifiers (id, service_id, name, description, price_adjustment_cents, duration_adjustment_minutes, is_required, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').bind(generateId(), serviceId, body.name, body.description || null, Math.round(parseFloat(body.price_adjustment || '0') * 100), parseInt(body.duration_adjustment_minutes, 10) || 0, body.is_required === 'on' ? 1 : 0, (maxOrder?.max_order || 0) + 1).run();
...
app.post('/services/:id/rules', async (c) => {
    ...
    const serviceId = c.req.param('id');
    const body = await c.req.parseBody();
    await db.prepare('INSERT INTO price_adjustment_rules (id, service_id, rule_type, adjustment_type, adjustment_value, direction, days_of_week, start_time, end_time, min_hours_ahead, max_hours_ahead, territory_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(generateId(), serviceId, body.rule_type, body.adjustment_type, body.adjustment_type === 'flat'
        ? Math.round(parseFloat(body.adjustment_value || '0') * 100)
        : parseFloat(body.adjustment_value || '0'), body.direction, body.days_of_week || null, body.start_time || null, body.end_time || null, body.min_hours_ahead ? parseInt(body.min_hours_ahead, 10) : null, body.max_hours_ahead ? parseInt(body.max_hours_ahead, 10) : null, body.territory_id || null).run();
    return c.redirect(`/admin/services/${serviceId}`);
});
...
app.post('/customers', async (c) => {
    ...
    const duplicate = await db.prepare(`SELECT id, first_name, last_name FROM customers
     WHERE (? IS NOT NULL AND LOWER(email) = ?)
        OR (? IS NOT NULL AND phone_e164 = ?)
     LIMIT 1`).bind(email, email, phoneE164, phoneE164).first();
    if (duplicate) {
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).bind(id, body.first_name, body.last_name, email, phoneRaw || null, phoneE164).run();
    ...
    if (line1 && city && state && postal) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
    `).bind(generateId(), id, line1, (typeof body.address_line_2 === 'string' && body.address_line_2.trim()) ? body.address_line_2.trim() : null, city, state, postal, (typeof body.address_lat === 'string' && body.address_lat.trim()) ? Number.parseFloat(body.address_lat) : null, (typeof body.address_lng === 'string' && body.address_lng.trim()) ? Number.parseFloat(body.address_lng) : null).run();
    }
...
app.post('/customers/import', async (c) => {
    ...
    for (const entry of parsed) {
        ...
        const existing = await db.prepare(`SELECT id FROM customers
       WHERE (? IS NOT NULL AND LOWER(email) = ?)
          OR (? IS NOT NULL AND phone_e164 = ?)
       LIMIT 1`).bind(entry.email, entry.email, phoneE164, phoneE164).first();
        if (existing)
            continue;
        await db.prepare(`INSERT INTO customers (id, first_name, last_name, email, phone, phone_e164)
       VALUES (?, ?, ?, ?, ?, ?)`).bind(generateId(), entry.first_name, entry.last_name, entry.email, entry.phone, phoneE164).run();
    }
    return c.redirect('/admin/customers');
});
app.get('/customers/:id', async (c) => {
    const id = c.req.param('id');
    if (id === 'new')
        return c.redirect('/admin/customers/new');
...
app.post('/customers/:id', async (c) => {
    ...
    const duplicate = await db.prepare(`SELECT id, first_name, last_name FROM customers
     ...
     LIMIT 1`).bind(id, email, email, phoneE164, phoneE164).first();
    if (duplicate) {
        const q = new URLSearchParams({ error: `A customer already exists: ${duplicate.first_name} ${duplicate.last_name}.` });
    ...
    WHERE id = ?
  `).bind(body.first_name, body.last_name, email, phoneRaw || null, phoneE164, id).run();
    const line1 = typeof body.address_line_1 === 'string' ? body.address_line_1.trim() : '';
    const city = typeof body.address_city === 'string' ? body.address_city.trim() : '';
    const state = typeof body.address_state === 'string' ? body.address_state.trim() : '';
    const postal = typeof body.address_postal === 'string' ? body.address_postal.trim() : '';
...
app.post('/team', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, body.first_name, body.last_name, body.email, body.phone || null, body.role || 'provider', body.can_be_auto_assigned === 'on' ? 1 : 0, body.is_active === 'on' ? 1 : 0).run();
...
app.post('/team/:id', async (c) => {
    ...
    WHERE id = ?
  `).bind(body.first_name, body.last_name, body.email, body.phone || null, body.role || 'provider', body.can_be_auto_assigned === 'on' ? 1 : 0, body.is_active === 'on' ? 1 : 0, id).run();
...
app.get('/team/:id', async (c) => {
    ...
    const id = c.req.param('id');
    if (id === 'new')
        return c.redirect('/admin/team/new');
...
app.post('/team/:id/hours', async (c) => {
    ...
    for (let day = 0; day <= 6; day++) {
        if (body[`day_${day}_enabled`] === 'on') {
            await db.prepare('INSERT INTO provider_weekly_hours (id, team_member_id, day_of_week, start_time, end_time) VALUES (?, ?, ?, ?, ?)').bind(generateId(), memberId, day, body[`day_${day}_start`], body[`day_${day}_end`]).run();
        }
    }
...
app.post('/team/:id/overrides', async (c) => {
    ...
    const memberId = c.req.param('id');
    const body = await c.req.parseBody();
    await db.prepare('INSERT INTO provider_date_overrides (id, team_member_id, date, is_available, start_time, end_time) VALUES (?, ?, ?, ?, ?, ?)').bind(generateId(), memberId, body.date, parseInt(body.is_available, 10), body.start_time || null, body.end_time || null).run();
    return c.redirect(`/admin/team/${memberId}`);
});
...
app.get('/jobs/new', async (c) => {
    ...
    if (!selectedTerritoryId && onlyTerritory)
        selectedTerritoryId = onlyTerritory.id;
    let services = [];
    if (selectedTerritoryId) {
        const servicesRes = await db.prepare('SELECT s.id, s.name, s.description, s.base_price_cents, s.base_duration_minutes FROM services s JOIN territory_services ts ON s.id = ts.service_id WHERE ts.territory_id = ? AND s.is_active = 1 ORDER BY s.name').bind(selectedTerritoryId).all();
    ...
    if (!selectedServiceId && onlyService)
        selectedServiceId = onlyService.id;
    ...
    if (!selectedProviderId && onlyProvider)
        selectedProviderId = onlyProvider.id;
    const props = {
        customer,
        territories,
        services,
...
app.post('/jobs/quick-create', async (c) => {
    ...
    if (!customerId || !territoryId || !serviceId || !date || !time) {
        ...
        if (territoryId)
            q.set('territory_id', territoryId);
        if (serviceId)
        ...
        if (addressLine1)
            q.set('address_line1', addressLine1);
    ...
    if (addressLine1) {
        ...
        await db
            .prepare('INSERT INTO customer_addresses (id, customer_id, line_1, city, state, postal_code, is_default) VALUES (?, ?, ?, ?, ?, ?, 1)')
    ...
    await db
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
        .bind(jobId, customerId, serviceId, territoryId, customerAddressId, date, time, duration, priceCents, priceCents, JSON.stringify([buildServiceBaseLine('Service', priceCents)]), providerId ? 'assigned' : 'created')
        .run();
    if (providerId) {
...
app.get('/api/customers/search', async (c) => {
    ...
    if (q.length < 2)
        return c.html('');
    const customers = await db.prepare('SELECT id, first_name, last_name, email FROM customers WHERE first_name LIKE ? OR last_name LIKE ? OR email LIKE ? ORDER BY last_name, first_name LIMIT 10').bind(`%${q}%`, `%${q}%`, `%${q}%`).all();
...
app.post('/api/customers/create-for-job', async (c) => {
    ...
    const phoneRaw = typeof body.phone === 'string' ? body.phone.trim() : '';
    const phoneE164 = normalizePhoneE164(phoneRaw || null);
    const existing = await db.prepare(`SELECT id, first_name, last_name, email, phone FROM customers
     WHERE (? IS NOT NULL AND LOWER(email) = ?)
        OR (? IS NOT NULL AND phone_e164 = ?)
     LIMIT 1`).bind(email, email, phoneE164, phoneE164).first();
    const id = existing?.id || generateId();
    if (!existing) {
        await db.prepare('INSERT INTO customers (id, first_name, last_name, email, phone, phone_e164) VALUES (?, ?, ?, ?, ?, ?)').bind(id, body.first_name, body.last_name, email, phoneRaw || null, phoneE164).run();
    }
    return c.html(JobWizardPage({
        ...
        customer: existing
            ...
            : { id, first_name: body.first_name, last_name: body.last_name, email: email || '', phone: phoneRaw }
    }));
});
app.post('/jobs/wizard/step1-address', async (c) => {
    ...
    try {
        ...
        await db.prepare('INSERT OR IGNORE INTO customer_addresses (id, customer_id, line_1, city, state, postal_code, lat, lng, is_default) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)').bind(addressId, state.customer_id, state.address_line1, state.address_city, state.address_state, state.address_postal, state.address_lat ? parseFloat(state.address_lat) : null, state.address_lng ? parseFloat(state.address_lng) : null).run();
        const services = await db.prepare('SELECT s.id, s.name, s.description, s.base_price_cents, s.base_duration_minutes FROM services s JOIN territory_services ts ON s.id = ts.service_id WHERE ts.territory_id = ? AND s.is_active = 1 ORDER BY s.name').bind(matchedTerritory.id).all();
...
app.post('/jobs/create', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(jobId, state.customer_id, state.service_id || null, state.territory_id, state.date, state.time, duration, priceCents, priceCents, JSON.stringify([buildServiceBaseLine('Service', priceCents)]), state.provider_id ? 'assigned' : 'created').run();
...
app.get('/jobs/:id', async (c) => {
    ...
    const id = c.req.param('id');
    if (id === 'new' || id === 'wizard')
        return c.redirect('/admin/jobs/new');
    ...
    const customerPhone = customer?.phone_e164
        || normalizePhoneE164(customer?.phone || null);
    const smsThreadMessage = await db.prepare(`SELECT id, is_read, updated_at, body,
     ...
     LIMIT 1`).bind(id, id, customerPhone || '').first();
    const assignedProviderId = (jobProviders.results || [])[0]?.id;
    const jobModel = job;
    const notes = JSON.parse(notesJson);
    const lineItems = parsedLineItems.length > 0
        ? parsedLineItems
        : [buildServiceBaseLine(service?.name || job.custom_service_name || 'Service', Number(job.total_price_cents || 0))];
...
app.get('/jobs/:id/sms-thread-card', async (c) => {
    ...
    const customer = job?.customer_id
        ? await db.prepare('SELECT phone, phone_e164 FROM customers WHERE id = ?').bind(job.customer_id).first()
        : null;
...
app.post('/jobs/:id/status', async (c) => {
    ...
    const jobData = await db.prepare(`SELECT j.customer_id, j.total_price_cents, j.scheduled_date, j.scheduled_start_time, j.line_items_json,
    ...
    if (jobData) {
        ...
        if (['assigned', 'enroute'].includes(status)) {
            const provider = await db.prepare(`SELECT tm.first_name, tm.last_name FROM job_providers jp
         JOIN team_members tm ON tm.id = jp.team_member_id
         WHERE jp.job_id = ? LIMIT 1`).bind(jobId).first();
            if (provider)
...
app.post('/jobs', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, body.customer_id, body.service_id || null, body.territory_id, body.scheduled_date, body.scheduled_start_time, parseInt(body.duration_minutes, 10) || 60, totalPriceCents, totalPriceCents, JSON.stringify([buildServiceBaseLine(String(body.custom_service_name || 'Service'), totalPriceCents)]), body.custom_service_name || null, body.status || 'created').run();
    return c.redirect('/admin/jobs');
});
...
app.post('/jobs/:id', async (c) => {
    ...
    if (section === 'details') {
        ...
        const existing = await db.prepare('SELECT custom_service_name, line_items_json FROM jobs WHERE id = ?').bind(id).first();
      ...
      WHERE id = ?
    `).bind(body.scheduled_date, body.scheduled_start_time, duration, basePriceCents, totalPriceCents, JSON.stringify([generated, ...customLines]), id).run();
    ...
    WHERE id = ?
  `).bind(body.customer_id, body.service_id || null, body.territory_id, body.scheduled_date, body.scheduled_start_time, parseInt(body.duration_minutes, 10) || 60, totalPriceCents, JSON.stringify([baseLine, ...customLines]), body.custom_service_name || null, body.status || 'created', id).run();
    return c.redirect(`/admin/jobs/${id}`);
});
...
app.get('/invoices/new', async (c) => {
    ...
    const fields = [
        ...
        { name: 'customer_id', label: 'Customer', type: 'select', required: true, options: (customers.results || []).map(c => ({ value: c.id, label: `${c.first_name} ${c.last_name}` })) },
...
app.post('/invoices', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, body.invoice_number || await nextInvoiceNumber(db), body.customer_id, body.job_id || null, body.currency || 'CAD', totalCents, subtotalCents, taxCents, discountCents, totalCents, JSON.stringify(lineItems), body.due_date || null, status, paidAt, body.notes || null).run();
    return c.redirect('/admin/invoices');
});
app.get('/invoices/:id', async (c) => {
    const id = c.req.param('id');
    if (id === 'new')
        return c.redirect('/admin/invoices/new');
...
app.post('/invoices/:id', async (c) => {
    ...
    WHERE id = ?
  `).bind(body.invoice_number, body.customer_id, body.job_id || null, body.currency || 'CAD', totalCents, subtotalCents, taxCents, discountCents, totalCents, JSON.stringify(lineItems), body.due_date || null, status, status, paidAt, body.notes || null, id).run();
    return c.redirect('/admin/invoices');
});
...
app.post('/recurring', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, body.customer_id, body.service_id, body.territory_id, body.frequency, body.day_of_week ? parseInt(body.day_of_week, 10) : null, body.scheduled_start_time || null, parseInt(body.duration_minutes, 10) || 60, totalPriceCents, totalPriceCents, body.is_active === 'on' ? 1 : 0).run();
    return c.redirect('/admin/recurring');
});
app.get('/recurring/:id', async (c) => {
    const id = c.req.param('id');
    if (id === 'new')
        return c.redirect('/admin/recurring/new');
...
app.get('/recurring/:id/edit', async (c) => {
    ...
    const fields = [
        { name: 'customer_id', label: 'Customer', type: 'select', required: true, value: recurring.customer_id, options: (customers.results || []).map(c => ({ value: c.id, label: `${c.first_name} ${c.last_name}` })) },
...
app.post('/recurring/:id', async (c) => {
    ...
    WHERE id = ?
  `).bind(body.customer_id, body.service_id, body.territory_id, body.frequency, body.day_of_week ? parseInt(body.day_of_week, 10) : null, body.scheduled_start_time || null, parseInt(body.duration_minutes, 10) || 60, totalPriceCents, body.is_active === 'on' ? 1 : 0, id).run();
    return c.redirect('/admin/recurring');
});
...
app.post('/push/subscribe', async (c) => {
    ...
    const preferencesRecord = (preferencesRaw && typeof preferencesRaw === 'object' && !Array.isArray(preferencesRaw))
        ...
        : {};
    try {
        await upsertPushSubscription(db, email, {
            endpoint,
            keys: { p256dh, auth },
        }, {
...
app.get('/push/pending', async (c) => {
    ...
    let notifications = [];
    try {
        notifications = await pullPendingPushNotifications(db, email, endpoint, Number.isFinite(limitRaw) ? limitRaw : 6);
    }
    catch (error) {
        return c.json({
            notifications: [],
...
app.get('/sms-settings', async (c) => {
    ...
    const smsStats = await db.prepare(`SELECT
       COUNT(*) as total,
...
app.post('/sms-settings', async (c) => {
    ...
    await db.prepare("INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES ('twilio_config', ?, datetime('now'))").bind(JSON.stringify(config)).run();
...
app.post('/sms-templates/:id', async (c) => {
    ...
    if (bodyTemplate) {
        await db.prepare("UPDATE sms_templates SET body_template = ?, is_active = ?, updated_at = datetime('now') WHERE id = ?").bind(bodyTemplate, isActive, id).run();
    }
    else {
        await db.prepare("UPDATE sms_templates SET is_active = ?, updated_at = datetime('now') WHERE id = ?").bind(isActive, id).run();
    }
    return c.body('', 200);
});
...
app.post('/coupons', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, body.code.toUpperCase(), body.discount_type, parseInt(body.discount_value, 10) || 0, body.max_uses ? parseInt(body.max_uses, 10) : null, body.valid_from || null, body.valid_until || null, body.is_active === 'on' ? 1 : 0).run();
    return c.redirect('/admin/coupons');
});
app.get('/coupons/:id', async (c) => {
    const id = c.req.param('id');
    if (id === 'new')
        return c.redirect('/admin/coupons/new');
...
app.post('/coupons/:id', async (c) => {
    ...
    WHERE id = ?
  `).bind(body.code.toUpperCase(), body.discount_type, parseInt(body.discount_value, 10) || 0, body.max_uses ? parseInt(body.max_uses, 10) : null, body.valid_from || null, body.valid_until || null, body.is_active === 'on' ? 1 : 0, id).run();
    return c.redirect('/admin/coupons');
});
...
app.post('/webhooks', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?)
  `).bind(id, body.url, body.event_type, secret, body.is_active === 'on' ? 1 : 0).run();
    return c.redirect('/admin/webhooks');
});
app.get('/webhooks/:id', async (c) => {
    const id = c.req.param('id');
    if (id === 'new')
        return c.redirect('/admin/webhooks/new');
...
app.post('/webhooks/:id', async (c) => {
    ...
    WHERE id = ?
  `).bind(body.url, body.event_type, body.is_active === 'on' ? 1 : 0, id).run();
    return c.redirect('/admin/webhooks');
});
...
async function getInboxSmsContext(db, messageId) {
    ...
    let smsHistory = [];
    if (phoneE164) {
        const rows = await db.prepare(`SELECT id, direction, body, status, created_at, segments
       FROM sms_log
       WHERE phone_to = ? OR phone_from = ?
       ORDER BY created_at ASC
       LIMIT 100`).bind(phoneE164, phoneE164).all();
...
async function getInboxJobContext(db, messageId) {
    ...
    if (!customerId && phoneE164) {
        const customer = await db.prepare(`SELECT id
       FROM customers
       WHERE phone_e164 = ? OR phone = ?
       ORDER BY updated_at DESC
       LIMIT 1`).bind(phoneE164, msg.phone || phoneE164).first();
        customerId = customer?.id || null;
    }
    if (!customerId) {
        return { jobOptions: [], selectedJobId: null };
    }
    const jobs = await db.prepare(`SELECT j.id, j.scheduled_date, j.status,
            COALESCE(s.name, j.custom_service_name, 'Service') as service_name
     FROM jobs j
     LEFT JOIN services s ON s.id = j.service_id
     WHERE j.customer_id = ?
     ORDER BY j.updated_at DESC
     LIMIT 20`).bind(customerId).all();
...
app.post('/inbox/:id/sms-reply', async (c) => {
    ...
    if (!smsBody) {
        ...
        if (result.success) {
            await db.prepare("UPDATE messages SET status = 'replied', replied_at = datetime('now'), updated_at = datetime('now') WHERE id = ?").bind(id).run();
        }
    }
    const twilioEnabled = await isTwilioEnabled(db);
    const { smsHistory } = await getInboxSmsContext(db, id);
...
app.post('/inbox/:id/sms-task', async (c) => {
    ...
    if (!smsLogId) {
        ...
        const sms = await db.prepare(`SELECT id, direction, body, created_at, phone_to, phone_from
       FROM sms_log
       WHERE id = ?
       LIMIT 1`).bind(smsLogId).first();
        const job = await db.prepare('SELECT notes_json FROM jobs WHERE id = ?').bind(jobId).first();
        if (!sms) {
