...
app.post('/submit', zValidator('json', messageSchema), async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(id, data.source, 'first_name' in data ? data.first_name : null, 'last_name' in data ? data.last_name : null, data.email, 'phone' in data ? data.phone : null, 'postal_code' in data ? data.postal_code : null, 'reason' in data ? data.reason : null, subject, 'body' in data ? data.body : null, 'metadata' in data && data.metadata ? JSON.stringify(data.metadata) : null).run();
    ...
    const messagePreview = (data.source === 'contact'
        ? data.body
        : subject).replace(/\s+/g, ' ').trim().slice(0, 160);
...
app.get('/', async (c) => {
    ...
    return c.json({
        ...
        next_cursor: result.results.length > 0
            ? result.results[result.results.length - 1].created_at
            : null,
    });
});
app.get('/:id', async (c) => {
