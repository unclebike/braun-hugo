...
async function ensureVapidConfig(db) {
    ...
    if (!publicJwk.x || !publicJwk.y)
        throw new Error('Failed to export VAPID public key');
    // Uncompressed EC point format: 0x04 || X || Y
    const publicBytes = concatBytes(new Uint8Array([0x04]), base64UrlDecode(publicJwk.x), base64UrlDecode(publicJwk.y));
...
async function createVapidJwt(audOrigin, subject, privateKeyJwk) {
    ...
    const sigDer = new Uint8Array(await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, key, new TextEncoder().encode(signingInput)));
...
async function ensurePushTables(db) {
    ...
    await db.prepare(`CREATE TABLE IF NOT EXISTS push_notification_queue (
      id TEXT PRIMARY KEY,
    ...
    await db.prepare('CREATE INDEX IF NOT EXISTS idx_push_subscriptions_user_email ON push_subscriptions(user_email)').run();
    await db.prepare('CREATE INDEX IF NOT EXISTS idx_push_subscriptions_endpoint ON push_subscriptions(endpoint)').run();
    await db.prepare('CREATE INDEX IF NOT EXISTS idx_push_queue_subscription_id ON push_notification_queue(subscription_id)').run();
}
...
export async function getPushVapidPublicKey(db) {
    const cfg = await ensureVapidConfig(db);
    return cfg.publicKey;
}
export async function upsertPushSubscription(db, staffEmail, subscription, _preferences) {
    ...
    await db.prepare(`INSERT INTO push_subscriptions (
     ...
     ON CONFLICT(endpoint) DO UPDATE SET
       ...
       updated_at = datetime('now')`).bind(crypto.randomUUID(), userEmail, endpoint, p256dh, auth, notifyNewJobs, notifyNewMessages).run();
}
export async function deactivatePushSubscription(db, staffEmail, endpoint) {
    ...
    await db.prepare(`UPDATE push_subscriptions
     ...
     WHERE user_email = ? AND endpoint = ?`).bind(userEmail, trimmed).run();
}
export async function getPushSubscriptionStatus(db, staffEmail, endpoint) {
    await ensurePushTables(db);
    const userEmail = staffEmail.trim().toLowerCase();
    const endpointTrimmed = (endpoint || '').trim();
    const row = endpointTrimmed
        ? await db.prepare(`SELECT notify_new_jobs, notify_new_messages
         FROM push_subscriptions
         WHERE user_email = ? AND endpoint = ? AND is_active = 1`).bind(userEmail, endpointTrimmed).first()
        : await db.prepare(`SELECT notify_new_jobs, notify_new_messages
         FROM push_subscriptions
         WHERE user_email = ? AND is_active = 1
         ORDER BY datetime(updated_at) DESC
         LIMIT 1`).bind(userEmail).first();
    if (!row) {
...
export async function pullPendingPushNotifications(db, staffEmail, endpoint, limit = 6) {
    const trimmed = endpoint.trim();
    if (!trimmed)
        return [];
    await ensurePushTables(db);
    ...
    const sub = await db.prepare(`SELECT id
     FROM push_subscriptions
     WHERE user_email = ? AND endpoint = ? AND is_active = 1`).bind(userEmail, trimmed).first();
    if (!sub?.id)
        return [];
    const pending = await db.prepare(`SELECT id, title, body, target_url, created_at
     FROM push_notification_queue
     WHERE subscription_id = ?
       AND delivered_at IS NULL
     ORDER BY datetime(created_at) ASC
     LIMIT ?`).bind(sub.id, limitClamped).all();
    const rows = pending.results || [];
    if (rows.length) {
        const ids = rows.map((r) => r.id);
        await db.prepare(`UPDATE push_notification_queue
...
export async function enqueueTestPushNotificationAndPing(db, staffEmail, endpoint) {
    const trimmed = endpoint.trim();
    if (!trimmed)
    ...
    await db.prepare(`INSERT INTO push_notification_queue (id, subscription_id, event_type, title, body, target_url)
     VALUES (?, ?, 'new_message', ?, ?, ?)`).bind(id, sub.id, 'GOATkit test notification', 'If you can see this, push delivery is working.', '/admin').run();
    const delivered = await pingPushEndpoint(db, trimmed);
    return { ...delivered, queued: true };
}
export async function enqueueAndDispatchPushEvent(db, event) {
    await ensurePushTables(db);
    ...
    const subs = await db.prepare(`SELECT id, endpoint
     FROM push_subscriptions
     WHERE is_active = 1
       AND (
    ...
    for (const row of rows) {
        inserts.push(db.prepare(`INSERT INTO push_notification_queue (id, subscription_id, event_type, title, body, target_url)
       VALUES (?, ?, ?, ?, ?, ?)`).bind(crypto.randomUUID(), row.id, type, title, body, targetUrl));
    }
