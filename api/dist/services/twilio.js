 ...
 */
// ─── Config ─────────────────────────────────────────────────────────────────
export async function getTwilioConfig(db) {
    try {
...
export async function isTwilioEnabled(db) {
 ...
 */
export async function sendSms(config, to, body, statusCallbackUrl) {
    if (!config.enabled) {
...
export async function ensureSmsInboxMessage(params) {
    ...
    const existingByJob = jobId
       ...
       LIMIT 1`).bind(jobId).first()
        : null;
    if (existingByJob?.id)
        return existingByJob.id;
    const existingByPhone = await db.prepare(`SELECT id, json_extract(metadata, '$.job_id') as existing_job_id
     FROM messages
     WHERE source = 'sms' AND phone = ?
     ORDER BY updated_at DESC
     LIMIT 1`).bind(phoneE164).first();
    if (existingByPhone?.id) {
        const existingJobId = existingByPhone.existing_job_id || null;
        const canReuseByPhone = (jobId && existingJobId === jobId) ||
            (!jobId && !existingJobId);
        if (!canReuseByPhone) {
         ...
         VALUES (?, 'sms', 'new', ?, ?, ?, ?, 'sms', ?, ?, ?, 0, datetime('now'), datetime('now'))`).bind(id, firstName || null, lastName || null, email || null, phoneE164, `SMS thread - ${phoneE164}`, '', JSON.stringify({ type: 'sms_thread', phone_e164: phoneE164, customer_id: customerId || null, job_id: jobId || null })).run();
            return id;
        }
        await db.prepare(`UPDATE messages
       SET metadata = ?, updated_at = datetime('now')
       WHERE id = ?`).bind(JSON.stringify({ type: 'sms_thread', phone_e164: phoneE164, customer_id: customerId || null, job_id: jobId || null }), existingByPhone.id).run();
...
export async function touchSmsInboxMessage(db, messageId, previewText) {
    await db.prepare(`UPDATE messages
     SET status = 'new',
         is_read = 0,
         read_at = NULL,
     ...
     WHERE id = ?`).bind(previewText, messageId).run();
}
export async function logSms(db, entry) {
    const id = entry.id || crypto.randomUUID();
    await db.prepare(`INSERT INTO sms_log
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`).bind(id, entry.customerId || null, entry.jobId || null, entry.messageId || null, entry.direction, entry.phoneTo, entry.phoneFrom, entry.body, entry.templateEvent || null, entry.twilioSid || null, entry.status, entry.errorCode || null, entry.errorMessage || null, entry.segments || 1).run();
    return id;
}
export async function updateSmsStatus(db, twilioSid, status, errorCode, errorMessage) {
    await db.prepare(`UPDATE sms_log SET status = ?, error_code = ?, error_message = ?, updated_at = datetime('now') WHERE twilio_sid = ?`).bind(status, errorCode || null, errorMessage || null, twilioSid).run();
}
// ─── Webhook Validation ─────────────────────────────────────────────────────
/**
...
export async function getCustomerSmsStatus(db, customerId) {
    const row = await db.prepare('SELECT sms_consent, sms_opted_out, phone_e164 FROM customers WHERE id = ?').bind(customerId).first();
    if (!row) {
 ...
 */
export function canSendAutomatedSms(status) {
 ...
 */
export async function sendJobSms(params) {
    ...
    // 4. Look up template
    const template = await db.prepare('SELECT body_template, is_active FROM sms_templates WHERE event_type = ?').bind(eventType).first();
    if (!template || !template.is_active) {
        console.log(`[sms] Template ${eventType} not found or disabled`);
        return null;
    }
    // 5. Get customer phone
    const customer = await db.prepare('SELECT phone_e164, phone FROM customers WHERE id = ?').bind(customerId).first();
 ...
 */
export async function sendDirectSms(params) {
