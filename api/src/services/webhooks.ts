...
export async function triggerWebhook(db: D1Database, eventType: string, payload: unknown): Promise<void> {
  const result = await db.prepare(
    `SELECT id, url, event_type, secret
     FROM webhooks
     WHERE is_active = 1
       AND event_type = ?`
  ).bind(eventType).all<WebhookRow>();
